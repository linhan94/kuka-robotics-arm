function X = GetObjectPosAndOrientation(clientID, vrep, obj_name,armJoints)
...
[~,pose(1)] = vrep.simxGetJointPosition(clientID ,armJoints(1), vrep.simx_opmode_blocking );
[~,pose(2)] = vrep.simxGetJointPosition(clientID ,armJoints(2), vrep.simx_opmode_blocking );
[~,pose(3)] = vrep.simxGetJointPosition(clientID ,armJoints(3), vrep.simx_opmode_blocking );
[~,pose(4)] = vrep.simxGetJointPosition(clientID ,armJoints(4), vrep.simx_opmode_blocking );
[~,pose(5)] = vrep.simxGetJointPosition(clientID ,armJoints(5), vrep.simx_opmode_blocking );
...
X = struct('position', pos, 'orientation', or,'pose',pose,'bbox_max', bboxmax, 'bbox_min', bboxmin);
